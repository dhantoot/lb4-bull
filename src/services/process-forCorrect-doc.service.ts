...
export default class ProcessForCorrectDocProvider implements Provider<Queue> {
  public queue: Queue;
  public job: Job;
  public data: any;
  public name: string;
  ...
  async forCorrectDocProcessor(job: Job) {
    try {
      this.job = job
      const {
        opts,
  ...
  async start() {
    try {
      this.queue.process(this.forCorrectDocProcessor);
